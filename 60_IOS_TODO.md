...
**Подход к решению (V2-миграция)** — на случай когда придут жалобы или
появится время:

1. **Telemetry для понимания масштаба:** добавить серверную метрику
   «количество пользователей с `country=RU` + `timeZone != 0`» — сколько
   реально в зоне риска. Если меньше 50 — точечная починка через
   поддержку. Если сотни-тысячи — нужна V2-миграция.

2. **Sentry detection на клиенте (опционально, до V2):** при загрузке
   маршрута проверять `Station.timeArrival` и `Passenger.timeArrival` на
   разумность — должны быть в окне `[BasicData.timeStartWork - 30min,
   BasicData.timeEndWork + 30min]`. Если за пределами — логировать в
   Sentry с user_id и route_id, **не править данные**. Это даст список
   конкретных кейсов для целевой починки.

3. **V2-миграция (когда решим что нужна):**
   - В `UserSettings` добавить поле `migrationV2Done: Boolean = false`
   - Новый метод `RouteUseCase.migrateTimestampsV2(offsetFromMoscow)`:
     для пользователей где `migrationV2Done = false`, обходит все
     маршруты, для каждого Station/Passenger:
     - Проверяет «разумность» (тот же критерий что в Sentry detection)
     - Если **неразумно** и расхождение точно `~offsetFromMoscow` —
       сдвигает на `offsetFromMoscow`
     - Если разумно — не трогает (значит либо новые данные, либо уже
       починенные)
     - Ставит `migrationV2Done = true`
   - Новые пользователи получают `migrationV2Done = true` сразу при
     регистрации — миграция для них не запускается

4. **Идемпотентность:** проверка «разумности» гарантирует что повторный
   запуск V2-миграции не сломает уже починенные данные. Это критично:
   если пользователь обновится несколько раз подряд, миграция может
   запуститься повторно из-за рассинхрона `migrationV2Done` (например
   при сбросе `UserSettings`).

5. **Известное ограничение:** V2-миграция использует **текущий**
   `UserSettings.timeZone` для расчёта offset'а. Если пользователь
   менял timezone после первого использования (например, переезжал
   между регионами) — миграция применит неправильный offset для части
   старых данных. Это редкий случай, но возможный. Принимаем как
   acceptable risk.

6. **Альтернатива — серверная починка:** если все старые маршруты
   синхронизированы на сервер, можно сделать серверный SQL-скрипт по
   таблице `route` (с join на `user.timezone`), который применит сдвиг
   ко всем `stations` JSON-полям и `passengers`. Плюс: не требует
   Android-релиза. Минус: затрагивает только синхронизированных
   пользователей; те у кого сбой sync — останутся с битыми данными.

⚠️ **Связанная задача:** при работе над V2 сразу проверить нужно ли
снимать `@Ignore` с двух тестов в `MigrateTimestampsTest.kt`
(`irkutskOffset_stations_timesShifted` / `_passenger_timesShifted`) —
они проверяют именно V2 поведение.
