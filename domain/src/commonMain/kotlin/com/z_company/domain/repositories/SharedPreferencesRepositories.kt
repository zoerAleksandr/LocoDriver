...
interface SharedPreferencesRepositories {
    ...
    /**
     * Курсор дельта-синхронизации маршрутов — граница «всё до сюда уже получено».
     *
     * Непрозрачная строка сервера, разбирать её на клиенте нельзя. Сохраняется
     * только после того, как ВСЕ страницы дельты обойдены и применены: иначе
     * пропущенный кусок изменений не вернётся уже никогда.
     *
     * Стирается вместе с выходом из аккаунта. Плюс страховка на случай, если
     * стереть забыли: курсор не отправляется, когда локальных маршрутов нет
     * (см. cursorForRequest в data_remote).
     */
    fun getRouteSyncCursor(): String?
    fun setRouteSyncCursor(value: String?)

    /**
     * «Настройки изменены локально и ещё не отправлены на сервер».
     * Ставится при каждом локальном сохранении настроек, снимается после успешной
     * выгрузки. Двусторонняя синхронизация («Синхронизация») пушит настройки на
     * сервер только когда флаг взведён — иначе устройство, не менявшее настройки,
     * не затрёт на сервере более свежие настройки с другого устройства (оно лишь
     * подтянет их). По умолчанию true, чтобы у существующих установок локальные
     * настройки один раз выгрузились при первой же синхронизации (без потери данных).
     */
    fun getSettingsSyncPending(): Boolean
    fun setSettingsSyncPending(value: Boolean)
