...
object PayrollPaymentCatalog {
    val entries: List<PayrollPaymentDefinition> = listOf(
        ...
        accrual(SalaryPaymentId.RESERVE, "052L", "ОплЗаРабНаОдиночСледЛоком", "Следование резервом"),
        accrual(SalaryPaymentId.HOLIDAY, listOf("035L", "076L"), "ОплВыхДнейСверхНорм / ДоплВыхПраздСверхНорм", "Праздничные"),
        accrual(SalaryPaymentId.AVERAGE, listOf("030A", "030B"), "ОплПоСредЗаработку", "Оплата по среднему"),
        accrual(SalaryPaymentId.UNDERWORK, "048A", "ОплНеВыполНормТрНед2/3тар", "Оплата недоработки"),
        ...
        accrual(SalaryPaymentId.BUSINESS_TRIP, listOf("030A", "030B"), "ОплПоСредЗаработку", "Командировка"),
        ...
        accrual(SalaryPaymentId.HEAVY_TRAIN, listOf("152L", "152P"), "ДоплРабЛокомБрТяжДлинПоез", "Тяжёлые поезда"),
        accrual(SalaryPaymentId.LONG_TRAIN, listOf("152L", "152P"), "ДоплРабЛокомБрТяжДлинПоез", "Длинносоставные поезда"),
        accrual(SalaryPaymentId.HEAVY_LONG_DISTANCE, emptyList(), "Доплата за ПДМ", "Доплата за ПДМ"),
        accrual(SalaryPaymentId.DOUBLED_TRAIN, "158L", "ДоплРабЛокомБрРабСоедПоезд", "Сдвоенные поезда"),
