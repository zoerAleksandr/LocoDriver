...
## Часовые пояса — ПРАВИЛО учёта времени

> Единое правило, как маршруты сохраняются и считаются по времени.
> Три разных часовых пояса для трёх разных задач — не путать.

**1. Ввод и сохранение времени + учёт месяца («отработано», норма) —
по МОСКОВСКОМУ времени (GMT+3).**
- Время явки/сдачи вводится и хранится (epoch ms) с привязкой к
  `UserSettings.displayTimeZone()` = `GMT+3` для RU/BY; для KZ — по
  местному (`getTimeZone(settings.timeZone)`, KZT).
- К какому месяцу относится маршрут (для «отработано»/нормы) определяется
  этим же поясом.
- **Все точки создания маршрута обязаны использовать один путь:**
  ручная форма — `AppDateTimePicker(timeZoneStr = displayTz)`; мастер
  «Заполнить месяц», планирование в Календаре, старый График —
  `DateAndTimeConverter.toEpochMillis(...)` (внутри тот же
  `displayTimeZone()`). Нельзя вводить время в системном/локальном поясе —
  только через эти пути, иначе сдвиг даты/суммы.

**2. Переходные маршруты (через границу месяца) —
по настройке в разделе «Норма/Регион».**
- Куда отнести часы маршрута, начавшегося в одном месяце и закончившегося
  в другом, задаёт `crossMonthTimezone` (enum `CrossMonthTimezone`:
  `MOSCOW` → GMT+3, `LOCAL` → местный). Это `crossMonthTZ` в
  `TimeCalculationContext`.

**3. Ночные и праздничные часы — по МЕСТНОМУ времени выбранного пояса.**
- Считаются по `localTZ` = `getTimeZone(settings.timeZone)` (реальный ЧП
  машиниста), не по московскому. См. `CalculateNightTime.kt` и
  `UtilsForEntities.getWorkingTimeOnAHoliday`.

**Где закреплено:** `TimeZoneUtils.displayTimeZone()` (п.1),
`TimeCalculationContext.from()` (п.2 `crossMonthTZ`, п.3 `localTZ`).
Мастер «Заполнить месяц» (`ScheduleWizardViewModel.apply`) сохраняет
корректно — использует `toEpochMillis` (п.1); ночные/праздничные/переход
считаются ниже по стеку из сохранённого instant (пп.2–3).
...
- **`POST /v1/client-info`** — авторизованный служебный вызов в начале sync.
  Тело: `{platform: "android"|"ios"|"pwa", version: String}`. Сервер хранит
  версии независимо по платформам в `user.app_versions` (JSONB) и ничего из
  этого поля не возвращает клиентам.
