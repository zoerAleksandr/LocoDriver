...
- **Структура**: фиксированная тональная шапка (`accentSoft`) → скроллящийся
  ...
  - **Контент-секции** (по наличию данных): Время работы (Явка/Перерыв/Сдача),
    ...
    «принял→сдал», рекуперация со знаком «−» в `success`; у тепловоза под
    литрами приёмки и сдачи показывается пересчёт в кг по коэффициенту каждой
    секции, а при наличии экипировки — отдельная строка с её объёмом в литрах
    и/или массой в кг), Поезда · N (шапка +
...
- Поля: средний час, районный коэффициент, северный коэффициент, зональная надбавка,
  ...
  НДФЛ, профсоюз, прочие удержания. Отдельное поле «Доплата за ПДМ (6000 т. и
  350 осей)» задаёт процент доплаты (по умолчанию 5%); критерии фиксированы и
  пользователем не редактируются.
...
| Все маршруты | месяц, фильтр, сортировка, вид, действия, quick view, PDF | §25.4 |
...
- Тап по заголовку месяца открывает такую же month/year sheet, как на Главной.
...
- Delete/copy/share/sync/quick-view/PDF повторяют контракты Главной. Любая ошибка
  не удаляет карточку оптимистично без возможности восстановления.
...
- Редактор серии: имя, тип тяги и четыре нормы приёмки/сдачи (§0). `0` сохраняется
  как `null`. Изменение имени из `TimeBottomSheet` синхронизируется обратно по §2.12.
...
- Локомотив: показ отопления, собственных нужд, другого рода тока и прочих полей;
  скрытие не обнуляет показания.
