...
- **Мастер заполнения графика**: на шаге выбора первого дня доступны стрелки
  переключения месяца. Если соседний предыдущий месяц ранее был заполнен через
  мастер, предлагается продолжить его график; при согласии первый день вручную
  выбирать не требуется, фаза цикла продолжается автоматически. «Выбрать заново»
  (и любое закрытие шторки) сбрасывает `canContinuePrevious` — иначе шторка
  остаётся в композиции и её scrim блокирует нажатия по всему экрану мастера.
