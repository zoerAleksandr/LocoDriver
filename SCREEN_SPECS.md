...
только по среднему часу (`businessTripMoney`). Это жёсткий инвариант итогового
`SalaryForRouteState`: даже если отдельный расчёт получил маршрут напрямую,
сдвоенный поезд, переотдых, пробег и любые другие доплаты принудительно обнуляются
и не отображаются в шторке.
...
5. **Станции следования**: список, каждая — имя (автодополнение), прибытие,
   ...
   - **Номер пути** (`trackNumber`, `track_number` в API): поле «Путь» открывает
     числовую клавиатуру по умолчанию (обычно путь — число), с переключателем на
     буквенную рядом с лейблом (иконка «Аб»/«123») — нужна для путей вида «3Г».
     Переключатель не запоминается между открытиями шторки.
   - **Флаг «Конечная»** (`isFinalStation`): отмечает конечную станцию маршрута
     поезда. Информационный — в расчётах не участвует, в списке станций
     отображается суффиксом «· конечная» рядом с названием.
   - **Флаг «Проходная»** (`isPassingStation`): станция без остановки. Когда
     включён — в шторке доступно только одно время («Проследование», хранится в
     `timeArrival`; `timeDeparture` принудительно `null`). В списке станций у такой
     строки время показывается ОДНО, по центру ширины, отведённой под
   ...
   - **Перегон** (между station[i] и station[i+1], данные хранятся на
     ...
     — поля «От»/«До» предзаполнены названиями соседних станций, но редактируемы
     (правка меняет сами station[i]/station[i+1]), плюс путь на перегоне и
     примечание. Путь/примечание перегона отображаются пилюлями рядом с временем в
     пути в списке станций. Километры на перегоне пока не вводятся.
...
ограничивается неотрицательным временем рабочей смены.
...
- **Доплата за пробег:** расчёт и отображение выполняются отдельно для каждого
  ...
  пересчитывает ранее сохранённые маршруты. Доплата входит в `totalChargedMoney`,
...
- Оплата командировки = все фактические часы командировочных маршрутов ×
...
3. Поле **email**.
4. Поле **пароль** — появляется, когда email валиден. В режиме входа под ним
   ссылка «Создать новый пароль» с 60-секундным кулдауном.
5. Кнопка **«Войти» / «Зарегистрировать»** — появляется, когда пароль не
   короче `MIN_LENGTH_PASSWORD`. В режиме регистрации активна только при обоих
   принятых чекбоксах.
6. Чекбоксы согласий (только в режиме регистрации): лицензионное соглашение и
   политика обработки ПДн, названия документов — кликабельные ссылки.
7. Разделитель **«или»**.
8. Кнопка **VK ID** (`OneTap`).
9. Подпись под ней: «Если аккаунта ещё нет, он будет создан автоматически»
...
- **Синхронизация**: в профиле отображается одна кнопка «Синхронизация»
  ...
  - Локальный новый/изменённый маршрут выгружается; новый/более свежий серверный
    ...
    успеха физически удаляется локально. Ранее синхронизированный маршрут,
    отсутствующий в успешно полученном серверном списке, считается удалённым на
    другом устройстве и удаляется локально, даже если после предыдущей
    синхронизации его успели изменить локально: удаление побеждает и маршрут не
    восстанавливается на сервере. Только маршрут без облачного маркера считается
    новым и выгружается при отсутствии на сервере. При ошибке получения списка
    merge и локальные удаления не выполняются. `GET /v1/route/` при пустом
