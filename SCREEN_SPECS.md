...
- **VK ID подтверждается access-токеном (с версии 3.0.4).** Личность пользователя
  ...
  - `VkAuthError.UserNotFound` переключает тумблер формы гостя с «Вход» на
    «Регистрация» — вместо тупика с ошибкой пользователь сразу попадает в
    регистрацию.
...
- Вход и привязка через VK подтверждаются VK access-токеном, ошибки разложены по
  ...
  этим VK нет» — это отдельное состояние (переход в регистрацию), а не ошибка
  пароля; «VK уже привязан к другому аккаунту» показывается текстом, а не
  молчаливым логом.
