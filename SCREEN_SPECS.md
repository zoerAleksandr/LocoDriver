...
- **Тарифная ставка** — особый случай. Хранится не в `SalarySetting`, а в
  ...
  - Синяя дата начала действия тарифа (в карточке ставки и внутри шторки)
    открывает выбор календарной даты (`setDateSetTariffRate`); шторка выбора
    месяцев при этом закрывается и возвращается после выбора даты — две модальные
    шторки одновременно не показываются. День `1` означает «нового тарифа внутри
    месяца нет» и очищает `dateSetTariffRate`.
