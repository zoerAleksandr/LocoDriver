...
5. **Станции следования**: список, каждая — имя (автодополнение), прибытие,
   ...
   - **Отображение перегона** — отдельная карточка между строками станций (по
     референсу), с непрозрачным фоном `surfaceBright`: карточка станций лежит на
     `secondary`, поэтому перегон читается как отдельный блок (полупрозрачный
     `segmentBackgroundColor` на нём почти не отличался). Содержимое разнесено по
     смыслу: **верхняя строка** — подпись «ПЕРЕГОН» слева и время в пути справа
     (mono), **нижняя** — номер пути отдельной пилюлей «N путь» и примечание
     текстом. Слева иконка, справа шеврон. У пустого перегона — пунктирная рамка,
     иконка «+» и подпись «примечание» акцентным цветом. Километры и число
     смежных путей не выводятся.
   ...
   - **Кнопка показа/скрытия перегонов** справа от заголовка «МАРШРУТ»
     ...
     так же, как порядок сортировки станций.
