...
- Полноэкранный экран поверх приложения после сплэша: заголовок (жирный),
  многострочный текст, внизу кнопка **«Понятно»** во всю ширину. Тема-адаптив
  (обёрнут в `LocoDriverTheme`), длинный текст прокручивается.
- Кнопка «Понятно» закрывает экран; повторно то же сообщение не показывается.
