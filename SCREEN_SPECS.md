...
- **Год**: hero-значение + помесячные столбцы (`StatMonthBar`, широкие — 14dp текущий/
  10dp предыдущий). Тап по столбцу открывает детализацию метрики «Отработанное
  время» (`onOpenDetailMonth` → `openDetail("worked", initialMonthIndex)`) сразу на
  этом месяце — не на декабре по умолчанию.
...
- Норму уменьшают все типы отвлечений (отпуск, больничный, курсы, донорские,
  уход за ребёнком-инвалидом, командировка, прочее) — рабочий день 8 ч,
