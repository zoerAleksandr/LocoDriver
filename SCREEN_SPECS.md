...
- **«Вторая ночь подряд»** (`computeNightWarn`): ночью считается присутствие в
  фиксированном окне 00:00–05:00 — работа по маршруту либо отдых в пункте оборота.
  Домашний отдых, пришедшийся на это окно, ночью не считается. При этом дневной
  домашний промежуток не разрывает две рабочие ночи на соседних календарных датах.
  Проверяются две соседние ночи и предыдущий маршрут в пределах 36 ч; один длинный
  маршрут также может захватить две ночи. Пользовательские ночные для оплаты на это
...
- **Трей «поездка с отдыхом в ПО»** (`buildAllRouteRows` → `TripGroupRow`):
  смежные в отсортированном списке маршруты, связанные отдыхом в ПО
  (`turnaroundRestBetween`: у раннего плеча стоит `restPointOfTurnover`, явка
  позднего строго позже сдачи раннего), сворачиваются в один общий фон.
  ...
  на три плеча, между каждыми соседними — свой коннектор со станцией и
  длительностью). Шапка трея: при одном отдыхе — «Поездка с отдыхом · ЧЧ:ММ»,
  при нескольких — «Поездка с N отдыхами · ЧЧ:ММ», где ЧЧ:ММ — сумма отработанного
  времени всех плеч (только отображение, ничего не пересчитывает).
