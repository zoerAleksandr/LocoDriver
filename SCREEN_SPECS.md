...
- Верхний отступ до заголовка вложенной шторки уменьшен; кнопка «Отмена» в
  списке также отображается увеличенным шрифтом.

### 2.11. Шапка и завершение

- Шторка не перетаскивается и не закрывается свайпом: штатные жесты
  `ModalBottomSheet` отключены (`sheetGesturesEnabled = false`), drag-handle не
  показывается.
- Заголовок: «Приёмка» или «Сдача». Справа — **«Закрыть»** для выхода без
  финального подтверждения и **«Готово»** (просто синий текст):
...
- **Немедленное сохранение (не только по «Готово»):** любое изменение времени
  ...
  асинхронной загрузке станции лишних записей нет). Поэтому данные сохраняются
  независимо от способа завершения шторки. «Окончание работы» (delivery) синхронизируется в
...
- `onSave` в форме локомотива вызывает `saveAcceptanceFromSheet` /
  ...
  через `onClose` («Готово», «Закрыть», системный Back или тап вне шторки).
