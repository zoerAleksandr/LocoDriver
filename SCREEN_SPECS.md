...
6. **НАПАРНИКИ** (лёгкий блок, вариант D) — **между плитками «Расчёт»/«Отдых» и
   ...
   иконки-контейнера и без кнопки удаления в строке. Управляется `isShowPartner`.
   «Добавить напарника» → `preSaveRoute()` → экран мультивыбора (§8.5).
...
- App bar «‹ Напарники». Заголовок `ВСЕ НАПАРНИКИ · N`. Строка: круглый аватар-инициалы
  (accentSoft фон / accent текст) · ФИО (600) · вторая строка `таб. N`(accent, mono) +
  примечание через « · ». Нижняя **filled-кнопка** «＋ Добавить напарника» (primary/onPrimary,
  не FAB). Пустое состояние: круг-иконка + «Пока нет напарников» + пояснение.
- **MANAGE** (из Настроек, `showPartnersManage`): trailing строки — шеврон → редактор записи.
  VM `PartnerListViewModel` (flow справочника, `delete`).
- **SELECT** (из формы маршрута, `showPartnerPicker(basicId)`): trailing — галочка (мультивыбор),
  в топ-баре «Готово». Отмечены напарники, уже добавленные в маршрут (по `sourcePartnerId`).
  По «Готово» — сверка: для отмеченных без копии создаём `RoutePartner`(копия + `sourcePartnerId`),
  для снятых копий — удаляем. VM `PartnerPickerViewModel(basicId)` (`toggle`, `confirm{...}`).
...
- App bar «‹ Напарник» + текстовая кнопка «Готово» (accent). Карточка под `ДАННЫЕ НАПАРНИКА`:
  ФИО (плейсхолдер «Фамилия Имя Отчество»), Табельный № (плейсхолдер «0000000», значение mono),
  Примечания. Под карточкой пояснение «Напарник сохранится в справочнике…». В edit — «Удалить
  напарника». **Кнопки «Выбрать из справочника» здесь нет** — экран только создаёт/правит запись.
