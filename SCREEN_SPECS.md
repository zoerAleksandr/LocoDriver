...
участвует в расчёте.
При получении фокуса полем настроек ЗП (ставки, средний час, проценты, пороги
списочных надбавок, удержания) его текущий текст выделяется целиком: первая
набранная цифра заменяет предзаполненное значение (`0` → `5`, а не `05`); дальше
ввод идёт обычным образом с курсором в конце.
