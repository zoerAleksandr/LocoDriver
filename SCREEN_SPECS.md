...
5. **Станции следования**: список, каждая — имя (автодополнение), прибытие,
   ...
   - **Отображение перегона** — отдельная карточка между строками станций (по
     референсу): иконка, «Перегон · время в пути», второй строкой «путь N ·
     примечание». У пустого перегона — пунктирная рамка, иконка «+» и подпись
     «примечание» акцентным цветом. Километры и число смежных путей не выводятся.
