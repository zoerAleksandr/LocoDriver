...
- **Синхронизация**: в профиле отображается одна кнопка «Синхронизация с облаком»
  ...
  - Локальный новый/изменённый маршрут выгружается; новый/более свежий серверный
    загружается. Локальный soft-delete вызывает `DELETE /v1/route/{id}` и после
    ...
    другом устройстве и удаляется локально. При ошибке получения списка merge и
    локальные удаления не выполняются.
