...
5. **Станции следования**: список, каждая — имя (автодополнение), прибытие,
   ...
   - **Перегон** (между station[i] и station[i+1], данные хранятся на
     ...
     «Например: по неправильному»). Путь/примечание перегона отображаются пилюлями
     рядом с временем в пути; у пустого перегона показывается подпись «перегон».
     Километры на перегоне пока не вводятся.
