...
- Полноэкранный экран поверх приложения после сплэша (вариант «Новость»):
  hero-иконка (ℹ) в тональном скруглённом квадрате, крупный заголовок (жирный),
  многострочный текст, внизу pill-кнопка **«Понятно»** во всю ширину. В правом
  верхнем углу — крестик закрытия. Тема-адаптив (обёрнут в `LocoDriverTheme`),
  длинный текст прокручивается.
...
- Кнопка «Понятно» и крестик закрывают экран; повторно то же сообщение не
  показывается.
...
- `POST /v1/announcements/` с заголовком `X-ADMIN-KEY` (upsert по `number`),
  `GET /v1/announcements/` — список. Ключ `ADMIN_API_KEY` в конфиге сервера
  (fail-closed: пусто ⇒ доступа нет).
