...
2. **ВРЕМЯ РАБОТЫ**: явка (`timeStartWork`), сдача (`timeEndWork`), перерыв
   ...
   Если сдача раньше явки, форма показывает предупреждение «Окончание работы
   раньше начала. Невозможно сохранить маршрут.», а все пути локального сохранения
   (явное, автоматическое и перед переходом в дочерний раздел) отклоняют запись.
   Если такая запись уже существует в локальной БД или пришла из старых данных,
   расчёт зарплаты для неё не падает и возвращает `0`.
...
3. **Характеристики**: расстояние (`distance`), вес (`weight`), оси (`axle`),
   ...
   - Та же нормализация — в `addTrainDataVersion` (значения из шторки станции
     попадают и в `dataVersions`, и в сам поезд).
...
5. **Станции следования**: список, каждая — имя (автодополнение), прибытие,
   ...
   - **Флаг «Проходная»** (`isPassingStation`), стандартный `Checkbox`: станция без
     ...
     прибытие+стоянку+отправление, а название станции — приглушённым цветом
     (alpha 0.55). **Введённое время отправления при включении флага не стирается** —
   ...
   - **Отображение перегона** — отдельная карточка между строками станций (по
     ...
     отдельной пилюлей «N путь» и примечание текстом; при пустых данных нижней
     строки нет вовсе. Слева иконка-карандаш (о редактировании говорит она, а не
   ...
   - **«Будущий перегон»** — тот, у которого ещё неизвестна вторая граница, из-за
     ...
     с иконкой локомотива, без шеврона и не открывается по клику. Условия:
...
8. **Вагонник** (`CarInspector`: `fullName`, `tabNumber`, `couplingTime`) — тем же
   add/remove-паттерном, что толкач/двойная тяга, в той же шторке «Настройки
   поезда» (`CarInspectorSection`). Осматривает и закрепляет автосцепку перед
...
Оба поля синхронизируются с сервером: `isWorkStartByArrival` — в таблице пассажиров,
`timeStartWorkBeforeArrival` — в `basicData` (колонка `route.time_start_work_before_arrival`,
миграция 029). Раньше резерв жил только локально и обнулялся при любом скачивании
маршрута — после этого выключение режима не возвращало прежнюю явку.
...
- При каждом открытии экрана при наличии авторизации и активной подписки тихо
  запускается двусторонняя облачная синхронизация с общим cooldown 5 минут и
  задержкой старта 1,5 секунды: сначала отображается и рассчитывается локальный
  список. Под верхним app bar на время
...
- **Структура**: фиксированная тональная шапка (`accentSoft`) → скроллящийся
  ...
  - **Контент-секции** (по наличию данных): Время работы (Явка/Перерыв/Сдача),
    ...
    **оба в `primary`**, приб не бледнее отпр). У станции с флагом «конечная» —
    ...
    под названием — строка
    «перегон · путь N · примечание», если у перегона перед этой станцией
    заполнены `segmentTrackNumber`/`segmentNotes`. Ниже графика — вспомогательная
    тяга и строка «Вагонник: ФИО · таб. N · прицепка HH:MM» (`CarInspector`).
...
Самостоятельные тарифные строки `018L` и `052L` не считаются надбавками и не
добавляются повторно через усреднённый остаток сверхурочной базы.
На экране настроек зарплаты под процентными полями показывается краткая база
выплаты: обычный тариф, `018L`, фактический поездной сегмент либо подтверждённая
база районного/северного коэффициента. Поле «Другие надбавки» явно обозначено как
постоянный процент тарифа обычной работы, а не фиксированная премия.
...
- Строки «Прочие надбавки» и «Прочие удержания» всегда находятся последними
  соответственно в списках начислений и удержаний.
...
- **Списки порогов** (добавить/изменить/удалить строку): доплаты за тяжеловесные
  ...
  Для длинносоставного поезда и удлинённого плеча введённое значение является
  строгой границей: доплата начинается только при значении **больше** порога,
  равенство порогу доплату не включает. Рядом с названиями этих двух настроек
  показывается информационная кнопка, открывающая пояснение с примером. Для
  тяжеловесного поезда сохранена включающая граница минимального веса (`>=`).
...
- **Режим планирования** (`enterRoutePlan`): выбор времени явки (чипы `TimeOption`
  ...
  минуты. При совпадении показывается шторка с действиями «Заменить» и «Оставить оба».
...
- **Шаг 1**: выбор/редактирование графика (тап по дню цикла → пикер типа;
  ...
  первый день цикла в месяце. **Шаг 2**: предпросмотр раскладки на месяц и
  флажок «Продлить на следующий месяц». При включённом флажке `apply()` создаёт
...
- **Синхронизация**: в профиле отображается одна кнопка «Синхронизация»
  ...
  - **Подтверждение массового удаления.** Если доля пропавших с сервера
    ...
    Полные локальные копии сразу помещаются в корзину с
    `remoteDeletionPending=true`. На экране «Профиль» подтверждения больше нет:
    ручная и фоновая синхронизация показывают подсказку открыть корзину. Там
    пользователь может восстановить маршруты либо принять удаление. Серверный
    DELETE не отправляется — маршрутов на сервере уже нет.
...
- В `Настройки → Корзина маршрутов` показываются количество элементов, дата
  удаления, действия «Восстановить» для записи и «Восстановить всё».
...
- «Очистить корзину» требует подтверждения с точным количеством. Физически
  ...
  позволяет потерять tombstone до успешной серверной синхронизации.
...
- В карточке «О приложении» доступно действие «Экспорт диагностики».
- Перед системным share sheet показывается точный перечень включённых данных:
  диагностический код, версии приложения/Android/БД, статус миграции, агрегаты
  миграции и типы последних технических событий.
