...
3. **Характеристики**: расстояние (`distance`), вес (`weight`), оси (`axle`),
   ...
   - Рядом с «У.Д.» показывается расчётная длина состава в метрах. Категория
     ...
     длину пассажирского вагона (`UserSettings.passengerWagonLengthMeters`, по
   ...
   - Шестерёнка в топ-баре открывает отдельный подраздел настроек **«Поезд»**.
     ...
     быть положительным и хранится локально в SQLDelight.
...
- **Жесты (прокрутка ≠ закрытие)**: свайп по скроллящемуся контенту только
  прокручивает список и **не закрывает** шторку. Область контента несёт
  `NestedScrollConnection`, который в `onPostScroll`/`onPostFling` поглощает
  остаток драга/флинга **вниз** после того, как `LazyColumn` упёрся в верх,
  чтобы он не дошёл до `ModalBottomSheet` и не потянул её к закрытию. Закрыть
  шторку можно: ручкой/шапкой (она неподвижна, драг по ней тянет шторку),
  затемнением, кнопкой «назад» или действием панели. Это защита от случайного
  закрытия при свайпе по списку.
...
- **Сортировка списков** (от нового к старому): **поезда** — по времени
  отправления первой станции (`stations.first().timeDeparture`) по убыванию;
  **локомотивы** — по времени приёмки (`timeStartOfAcceptance`) по убыванию.
  Элементы без соответствующего времени уходят в конец списка в исходном
  порядке. Причина: порядок `route.trains`/`route.locomotives` из БД/сети не
  гарантирован (нет `ORDER BY`, `insertOrReplace` может переставить строки).
