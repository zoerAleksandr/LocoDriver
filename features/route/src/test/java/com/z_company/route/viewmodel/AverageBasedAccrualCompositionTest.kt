@file:OptIn(kotlin.time.ExperimentalTime::class)

package com.z_company.route.viewmodel

import com.z_company.domain.entities.Day
import com.z_company.domain.entities.MonthOfYear
import com.z_company.domain.entities.ReleaseType
import com.z_company.domain.entities.TagForDay
import com.z_company.domain.entities.setting.SalarySetting
import com.z_company.domain.entities.setting.UserSettings
import com.z_company.domain.entities.route.BasicData
import com.z_company.domain.entities.route.Route
import kotlinx.coroutines.flow.first
import kotlinx.coroutines.test.runTest
import kotlinx.datetime.LocalDateTime
import kotlinx.datetime.TimeZone
import kotlinx.datetime.toInstant
import kotlin.test.Test
import kotlin.test.assertEquals

class AverageBasedAccrualCompositionTest {
    private val moscow = TimeZone.of("GMT+3")

    private fun instant(hour: Int): Long =
        LocalDateTime(2025, 1, 10, hour, 0).toInstant(moscow).toEpochMilliseconds()

    @Test
    fun averageChildCareAndTechnicalStudyUseSeparateHoursAndEnterTotalOnce() = runTest {
        val helper = SalaryCalculationHelper(
            userSettings = UserSettings(
                selectMonthOfYear = MonthOfYear(
                    year = 2025,
                    month = 0,
                    tariffRate = 100.0,
                    days = listOf(
                        Day(1, TagForDay.WORKING_DAY, true, ReleaseType.Other),
                        Day(2, TagForDay.SHORTENED_DAY, true, ReleaseType.ChildCare),
                        Day(
                            dayOfMonth = 3,
                            tag = TagForDay.WORKING_DAY,
                            isReleaseDay = true,
                            releaseType = ReleaseType.TechnicalStudy,
                            hours = 2.5,
                        ),
                    ),
                ),
            ),
            salarySetting = SalarySetting(
                averagePaymentHour = 200.0,
                zonalSurcharge = 0.0,
                harmfulnessPercent = 0.0,
            ),
            allRoutes = emptyList(),
        )

        assertEquals(8 * 3_600_000L, helper.getDayOffHoursFlow().first())
        assertEquals(1_600.0, helper.getMoneyAverageFlow().first(), 0.001)
        assertEquals(7 * 3_600_000L, helper.getHoursCaringForDisableChildren().first())
        assertEquals(1_400.0, helper.getMoneyCaringForDisableChildren().first(), 0.001)
        assertEquals((2.5 * 3_600_000).toLong(), helper.getTechnicalStudyTimeFlow().first())
        assertEquals(500.0, helper.getMoneyTechnicalStudyFlow().first(), 0.001)
        assertEquals(3_500.0, helper.getMoneyTotalChargedFlow().first(), 0.001)
    }

    @Test
    fun underworkEntersTotalAndCanBeDisabled() = runTest {
        fun helper(enabled: Boolean) = SalaryCalculationHelper(
            userSettings = UserSettings(
                selectMonthOfYear = MonthOfYear(
                    year = 2025,
                    month = 0,
                    tariffRate = 100.0,
                    days = listOf(Day(10, TagForDay.WORKING_DAY)),
                ),
                timeZone = 0L,
            ),
            salarySetting = SalarySetting(
                averagePaymentHour = 200.0,
                showUnderworkPayments = enabled,
                zonalSurcharge = 0.0,
                harmfulnessPercent = 0.0,
            ),
            allRoutes = listOf(Route(basicData = BasicData(
                timeStartWork = instant(8),
                timeEndWork = instant(11),
            ))),
            effectiveNormaHoursForUnderwork = 8,
        )

        val enabled = helper(true)
        assertEquals(5 * 3_600_000L, enabled.getUnderworkTimeFlow().first())
        assertEquals(1_000.0, enabled.getMoneyUnderworkFlow().first(), 0.001)
        assertEquals(1_300.0, enabled.getMoneyTotalChargedFlow().first(), 0.001)

        val disabled = helper(false)
        assertEquals(0L, disabled.getUnderworkTimeFlow().first())
        assertEquals(300.0, disabled.getMoneyTotalChargedFlow().first(), 0.001)
    }

    @Test
    fun technicalStudyClosesUnderworkHoursWithoutBeingPaidTwice() = runTest {
        val helper = SalaryCalculationHelper(
            userSettings = UserSettings(selectMonthOfYear = MonthOfYear(
                year = 2025,
                month = 0,
                tariffRate = 100.0,
                days = listOf(Day(
                    dayOfMonth = 10,
                    tag = TagForDay.WORKING_DAY,
                    isReleaseDay = true,
                    releaseType = ReleaseType.TechnicalStudy,
                    hours = 2.5,
                )),
            )),
            salarySetting = SalarySetting(
                averagePaymentHour = 200.0,
                zonalSurcharge = 0.0,
                harmfulnessPercent = 0.0,
            ),
            allRoutes = emptyList(),
            effectiveNormaHoursForUnderwork = 8,
        )

        assertEquals((5.5 * 3_600_000).toLong(), helper.getUnderworkTimeFlow().first())
        assertEquals(1_100.0, helper.getMoneyUnderworkFlow().first(), 0.001)
        assertEquals(500.0, helper.getMoneyTechnicalStudyFlow().first(), 0.001)
        assertEquals(1_600.0, helper.getMoneyTotalChargedFlow().first(), 0.001)
    }
}
