...
class SalaryTotalCompositionTest {
    ...
    @Test
    fun totalChargedEqualsAllSimultaneouslyApplicableAccrualRows() = runTest {
        val train = Train(
            number = "2503",
            weight = "7000",
            axle = "351",
            ...
            stations = mutableListOf(
                Station(timeDeparture = instant(8)),
                Station(timeArrival = instant(12)),
            ),
        )
        val route = Route(
            basicData = BasicData(
        ...
        val visibleAccruals = listOf(
            helper.getMoneyAtWorkTimeAtTariff().first(),
            helper.getMoneyAtSingleLocomotiveFlow().first(),
            helper.getMoneyAtPassengerFlow().first(),
            helper.getMoneyAtPassengerOutsideWorkFlow().first(),
            helper.getMoneyAtNightTimeFlow().first(),
            helper.getMoneyAtHolidayFlow().first(),
            ...
            helper.getMoneyDoubledTrainSecondSurchargeFlow().first(),
            ...
            helper.getMoneyLinearMileageFlow().first(),
            helper.getMoneyOverRestFlow().first(),
        )

        assertTrue(visibleAccruals.count { it > 0.0 } >= 11)
        assertEquals(visibleAccruals.sum(), helper.getMoneyTotalChargedFlow().first(), 0.001)
    }
}
