# AGENTS.md — LocoDriver

> **Этот файл читается Codex в начале каждой сессии.** Здесь —
> правила, которые Codex должен соблюдать ВСЕГДА, и контекст,
...
### 0. Синхронизация со SCREEN_SPECS.md

В корне проекта есть **`SCREEN_SPECS.md`** — эталонное описание поведения
экранов (что видит пользователь + где/как хранятся и обрабатываются данные).
По нему воспроизводят логику iOS- и PWA-версии.

**Правило: любое изменение кода, затрагивающее поведение описанного там
экрана (UI, расчёты, хранение, контракт), ОБЯЗАНО сопровождаться правкой
`SCREEN_SPECS.md` в том же коммите.** Если тронутый экран ещё не описан —
добавить его описание. Расхождение кода и `SCREEN_SPECS.md` = баг.
Сейчас описаны: экран «Локомотив» (FormLocoScreen) и шторка установки
времени (TimeBottomSheet).
...
В Codex.ai Project (не в репозитории):
